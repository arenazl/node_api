            ...
            loadScript('/js/services_ui/common/simple-example-generator.js', function() {
                console.log('[Services UI] Módulo de ejemplos cargado');
                                ...
                                loadScript('/js/services_ui/ida/ida-service-handlers.js', function() {
                                    ...
                                    // Intentar cargar el módulo de visualización de estructura de solicitud
                                    const requestStructureScript = '/js/services_ui/ida/request-structure-display.js';
                                    const scriptExists = document.querySelector(`script[src="${requestStructureScript}"]`);

                                    if (scriptExists) {
                                        console.log('[Services UI] Módulo de visualización de estructura ya cargado, continuando...');
                                        loadVueltaModules();
                                    } else {
                                        // Verificar si el archivo existe antes de intentar cargarlo
                                        fetch(requestStructureScript)
                                            .then(response => {
                                                if (response.ok) {
                                                    loadScript(requestStructureScript, function() {
                                                        console.log('[Services UI] Módulo de visualización de estructura de solicitud cargado');
                                                        loadVueltaModules();
                                                    });
                                                } else {
                                                    console.warn('[Services UI] El archivo request-structure-display.js no existe, continuando sin él');
                                                    loadVueltaModules();
                                                }
                                            })
                                            .catch(error => {
                                                console.warn('[Services UI] Error al verificar request-structure-display.js:', error);
                                                loadVueltaModules();
                                            });
                                    }
                                });

                                // Función para cargar los módulos VUELTA después de IDA
                                function loadVueltaModules() {
