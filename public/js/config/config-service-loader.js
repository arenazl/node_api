...
const ConfigServiceLoader = {
    ...
    fetchHeaderSample: async function(serviceNumber) {
        ...
        try {
            ...
            console.error(`Error al obtener header sample para el servicio ${serviceNumber}:`, err);
            return null;
        }
    },
    ...
    loadSavedConfigurations: function(serviceNumber, onLoadCallback) {
        ...
        fetch(url)
            ...
            .catch(err => {
                ...
                ConfigUtils.showNotification(`Error al cargar configuraciones: ${err.message}`, 'error');
    ...
    loadSavedConfiguration: function(configId, onLoadCallback) {
        ...
        fetch(`/service-config/get/${configId}`)
            ...
            .catch(err => {
                ...
                ConfigUtils.showNotification(`Error al cargar configuración: ${err.message}`, 'error');
                
                // Call callback with error
                if (typeof onLoadCallback === 'function') {
                    onLoadCallback(null, err.message);
                }
            });
    }
};
