...
async function uploadExcelFile(formData) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        // Usar SweetAlert para mostrar mensaje de error de duplicado
        if (typeof Swal !== 'undefined') {
          await Swal.fire({
            title: 'Archivo duplicado',
            html: `<div style="text-align: left">
                   <strong>La versión que está intentando subir es idéntica a una ya existente.</strong><br><br>
                   ${errorData.message || 'El sistema ha detectado que este archivo ya ha sido procesado anteriormente.'}<br><br>
                   <em>No es necesario volver a subir este archivo.</em>
                   </div>`,
            icon: 'warning',
          ...
          alert(`Archivo duplicado: ${errorData.error || 'La versión que está intentando subir es idéntica a una ya existente'}`);
        }
        
        // Mostrar mensaje en consola pero no lanzar error para evitar la notificación de error
        ...
        // Retornar sin lanzar error para evitar el manejo genérico
        return;
      }
    ...
    ConfigUtils.showNotification(data.message, 'success');
    ...
    if (data.warnings) {
      ...
      try {
        ...
        ConfigUtils.showNotification("Error al cargar la estructura del archivo", "error");
      }
    ...
    setTimeout(() => {
      try {
        console.log("Activando pestañas para mostrar el contenido...");
        // Primero activar la pestaña principal 'carga'
        const cargaTab = document.querySelector('.main-tab-btn[data-tab="carga"]');
        if (cargaTab) {
          cargaTab.click();
          console.log("Pestaña 'carga' activada");
          
          // Luego activar las subpestañas para mostrar cada sección
          // Activar primero la cabecera como vista inicial
          const cabeceraTab = document.querySelector('[data-subtab="cabecera"]');
          if (cabeceraTab) {
            cabeceraTab.click();
            console.log("Subpestaña 'cabecera' activada");
          }
          
          // Activar y comprobar las demás pestañas una por una para asegurar que estén cargadas
          setTimeout(() => {
            const reqTab = document.querySelector('[data-subtab="requerimiento"]');
            if (reqTab) {
              reqTab.click();
              console.log("Subpestaña 'requerimiento' activada");
              
              // Verificar si se muestra contenido o el mensaje de vacío
              const requestTable = document.getElementById('requestTable');
              if (requestTable && requestTable.innerHTML.includes('Seleccione un archivo Excel')) {
                console.warn("La tabla de requerimiento no muestra datos!");
              }
              
              setTimeout(() => {
                const respTab = document.querySelector('[data-subtab="respuesta"]');
                if (respTab) {
                  respTab.click();
                  console.log("Subpestaña 'respuesta' activada");
                  
                  setTimeout(() => {
                    const jsonTab = document.querySelector('[data-subtab="json"]');
                    if (jsonTab) {
                      jsonTab.click();
                      console.log("Subpestaña 'json' activada");
                      
                      // Volver a la pestaña requerimiento para que sea la que queda visible
                      setTimeout(() => {
        ...
        console.error("Error al activar pestañas:", error);
      }
    ...
    // Ocultar el overlay de progreso inmediatamente una vez que el contenido está cargado
      ...
      if (progressOverlay) {
        ...
        if (window.progressTimeoutId) {
          ...
          console.log("Timeout de progreso cancelado porque el proceso completó exitosamente");
        }
      }
