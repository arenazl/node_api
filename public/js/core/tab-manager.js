...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateStructuresPanelVisibility() {
    const cargaTab = document.getElementById('carga');
    const jsonSubtab = document.getElementById('subtab-json');
    const structuresPanel = document.getElementById('structuresPanel');
    
    // Verificar si los elementos existen antes de acceder a sus propiedades
    if (cargaTab && jsonSubtab && structuresPanel) {
      const cargaTabActive = cargaTab.classList.contains('active');
      const jsonSubtabActive = jsonSubtab.classList.contains('active');
  ...
  // Ejecutar la configuración de eventos
  setupTabEvents();
