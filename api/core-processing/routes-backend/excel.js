...
router.post('/upload', async (req, res) => {
  try {
    ...
    try {
      ...
      // [REMOVIDO] Integración con Mora.Sim-api movida al orchestrator step2
      // La llamada a Mora.Sim-api ahora se hace desde el frontend via step2
      // para mantener visibilidad de red (2 llamadas HTTP separadas)
      ...
      console.log('[EXCEL] Limpiando caché de servicios para actualizar combos...');
...
router.get('/structure', async (req, res) => {
  try {
    ...
    // NUEVO: Intentar obtener el ExcelId más reciente para este servicio
    let excelId = null;
    try {
      // Extraer el número de servicio del nombre del archivo de estructura
      const serviceNumberMatch = structure_file.match(/(\d{4})/);
      if (serviceNumberMatch) {
        const serviceNumber = serviceNumberMatch[1];
        
        // Buscar el ExcelId en el archivo de mapeo
        const mappingDir = path.join(__dirname, '../../../JsonStorage', 'excel-mappings');
        const mappingFile = path.join(mappingDir, `${serviceNumber}.json`);
        
        if (fs.existsSync(mappingFile)) {
          try {
            const mappingData = fs.readJsonSync(mappingFile);
            excelId = mappingData.excelId;
            console.log(`📝 [EXCEL] ExcelId encontrado en mapeo: ${excelId} para servicio ${serviceNumber}`);
          } catch (err) {
            console.log(`⚠️ [EXCEL] Error leyendo archivo de mapeo: ${err.message}`);
          }
        } else {
          console.log(`⚠️ [EXCEL] No se encontró archivo de mapeo para servicio ${serviceNumber}`);
        }
      }
    } catch (error) {
      console.log('⚠️ [EXCEL] No se pudo obtener ExcelId:', error.message);
    }
    
    // Devolver la estructura con el ExcelId si está disponible
    res.json({
      ...structure,
      excelId: excelId
    });
...
router.get('/structure-by-service', async (req, res) => {
  try {
    ...
    console.log(`🔍 Buscando estructuras para servicio ${service_number} en ${structuresDir}`);
    
    const structureFiles = fs.readdirSync(structuresDir)
      .filter(file => {
        // Buscar archivos que contengan el número de servicio
        // Formatos posibles: 
        // - 1033_structure.json
        // - SM_1033_v1_structure.json
        // - 20250807T035625_1033_v1_structure.json (formato con timestamp)
        const hasStructureExtension = file.endsWith('_structure.json') || file.endsWith('structure.json');
        const hasServiceNumber = file.includes(`${service_number}_`) || 
                                file.includes(`_${service_number}_`) ||
                                file.includes(`_${service_number}.`) ||
                                file.startsWith(`${service_number}_`) ||
                                file.startsWith(`${service_number}.`);
        
        if (hasStructureExtension && hasServiceNumber) {
          console.log(`✅ Encontrado archivo de estructura: ${file}`);
          return true;
        }
        return false;
      });
    ...
    if (structureFiles.length === 0) {
      // NUEVO: Intentar buscar en los mapeos de Excel si existe
      const mappingDir = path.join(__dirname, '../../../JsonStorage', 'excel-mappings');
      const mappingFile = path.join(mappingDir, `${service_number}.json`);
      
      if (fs.existsSync(mappingFile)) {
        try {
          const mapping = fs.readJsonSync(mappingFile);
          console.log(`📝 Encontrado mapeo para servicio ${service_number}, buscando estructura asociada...`);
          
          // Buscar el archivo de estructura asociado
          const possibleStructureFile = `${mapping.serviceNumber}_structure.json`;
          const structurePath = path.join(structuresDir, possibleStructureFile);
          
          if (fs.existsSync(structurePath)) {
            const structure = fs.readJsonSync(structurePath);
            return res.json({
              structure: {
                header_structure: structure.header_structure || structure.headerStructure,
                service_structure: structure.service_structure || structure.serviceStructure
              },
              filename: possibleStructureFile,
              source: 'mapping'
            });
          }
        } catch (error) {
          console.error('Error leyendo mapeo:', error);
        }
      }
    ...
    // NUEVO: Intentar obtener el ExcelId del archivo de mapeo si existe
    try {
      const mappingDir = path.join(__dirname, '../../..', 'JsonStorage', 'excel-mappings');
      const mappingFile = path.join(mappingDir, `${service_number}.json`);
      ...
      if (fs.existsSync(mappingFile)) {
        const mappingData = fs.readJsonSync(mappingFile);
        if (mappingData.excelId) {
          structure.excelId = mappingData.excelId;
          console.log(`📝 [EXCEL] ExcelId ${mappingData.excelId} incluido para servicio ${service_number} desde mapeo`);
        }
      } else {
        console.log(`⚠️ [EXCEL] No se encontró archivo de mapeo para servicio ${service_number}`);
      }
    } catch (mappingError) {
      console.log(`⚠️ [EXCEL] Error leyendo mapeo para servicio ${service_number}:`, mappingError.message);
    }
