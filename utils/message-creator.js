...
function formatValue(value, length, type, fieldName = '', fieldValues = '') {
  // Si el valor es una cadena que parece tener un formato como "1. consulta de Alertas"
  // y el tipo es numérico, extraer solo el número
  if (typeof value === 'string' &&
      (type === 'numerico' || type === 'numeric' || type === 'numérico') &&
      value.match(/^\d+\.\s+/)) {
    // Extraer el número al principio
    const numMatch = value.match(/^(\d+)\./);
    if (numMatch && numMatch[1]) {
      value = numMatch[1]; // Usar solo el número
    }
  }
